...
void drawTrainStation(GLBI_Engine& myEngine, float posX, float posY)
{
    ...
    myEngine.mvMatrixStack.addTranslation(Vector3D(-W/2.0f, -D/2.0f, 0.0f)); // centering it inside a square
    myEngine.updateMvMatrix();
    ...
    // wood planks
    int nb_planks = 12;
    float plank_h = H / nb_planks;

    for (int i = 0; i < nb_planks; i++) 
    {
        float z = plank_h * i + plank_h / 2.0f;
        drawBox(myEngine, W/2,   -0.1f, z, W+0.2f,   0.3f, plank_h*0.8f, PLANK_R, PLANK_G, PLANK_B);
        drawBox(myEngine, W/2, D+0.1f, z, W+0.2f,   0.3f, plank_h*0.8f, PLANK_R, PLANK_G, PLANK_B);
        drawBox(myEngine, -0.1f,  D/2, z,   0.3f, D+0.2f, plank_h*0.8f, PLANK_R, PLANK_G, PLANK_B);
        drawBox(myEngine, W+0.1f, D/2, z,   0.3f, D+0.2f, plank_h*0.8f, PLANK_R, PLANK_G, PLANK_B);
    }
